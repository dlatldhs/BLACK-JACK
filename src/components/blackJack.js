...
export default class Blackjack extends React.Component { // react component 사용
    ...
    dealCards(deck)  {
        ...
        // getRandomCard 에서 randomCard 하고 updatedDeck을 반환함
        // randomCard는 deck(카드) 중에서 아무거나 랜덤으로 고른 하나이고 updateDeack 은 
        const dealerCard1 = this.getRandomCard(playerCard1.updatedDeck); // playerCard1 에 randomCard 랑 updatedDeck 이 있는데 updatedDeck => deck - randomCard
    ...
    startNewGame(type) {
        ...
        if ( type === 'continue' ) { //type 이 continue 일 때 (계속 하는거)
            if ( this.state.money > 0 ) { // 돈이 0 보다 크게 있음
                ...
                // dealCards 에서 updatedDeck(Deck - randomCard) player dealer 반환함
                const { updatedDeck , player , dealer } = this.dealCards(deck); // player 하고 dealer 한테 카드 주는거
            ...
            const { updatedDeck, player, dealer } = this.dealCards(deck); // 카드 할당
    ...
    hit() {
        ...
        if ( !this.state.gameOver ) { // gmaeOver 한 상태라면 실행 X gameOver 안했으면 실행 ㄱㄱ
            if ( this.state.currentChip ) { // chip(game coin)이 있을 때
                // getRandomCard 하면 randomCard 랑 updatedDeck 을 주는데 randomCard 는 말그대로 랜덤 카드이고 updatedDeck은 deck에서 randomCard index slpice 해서 뺀 거
    ...
    stand() {
        ...
        if ( !this.state.gameOver ) { // if not gameOVer
            const randomCard = this.getRandomCard(this.state.deck);// randomCard <= {randomCard , updatedDeck }
            let deck = randomCard.updatedDeck; // deck에 updatedDeck 넣음
